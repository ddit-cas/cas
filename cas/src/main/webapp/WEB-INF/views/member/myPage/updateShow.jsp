	...
	<form name="tx_editor_form" style="width: 100%;height: auto;" id="tx_editor_form" action="/cas/member/insertPromotion" method="post" accept-charset="utf-8" enctype="multipart/form-data">
		...
		<div style="float:left; width: 40%;">
			...
			<div id="imgDiv">
				<div class="imgDiv" style="width: 100%; height: 400px; border:1px solid #ccc; border-radius:0.8em;  ">
					<img id="posterImage" src="${promotionVO.contentImg }">
				</div>
			...
			<div>
				...
				<textarea name='consertContent' style="width: 100%;height: 221px;" placeholder="(400자 이내로 기술하시오)">${promotionVO.consertContent }</textarea>
			</div>
		</div>
		...
		<div style="padding-left: 42%;">
			<div class="form-group">
				<label>제목</label> <input type="text" value="${promotionVO.contentTitle }" name="contentTitle" class="form-control showInsertText" id="email" placeholder="제목을 입력하세요.">
			</div>
			<div class="form-group">
				<div>
					<label>장르</label>
				...
				<div>
				    <select class="form-control genreSelect" name="consertGenre" style="width: 48%; float:left; margin-right: 1%;">
				    	<c:forEach  var="genre" items="${genreList}">
				    		<c:choose>
				    			<c:when test="${genre.genreName eq promotionVO.consertGenre }">
					    			<option value="${genre.genreCode }" selected="selected">${genre.genreName }</option>
				    			</c:when>
				    			<c:otherwise>
					...
					<input name="consertTime" type="number" min="0" value="${promotionVO.consertTime }" class="form-control" style="width:48%;" step="5" placeholder="분(m)단위로 입력하세요.">
				</div>
			</div>
			<div class="form-group">
			...
			<div style="margin-bottom: 15px;">
				<input  type="date" value="${promotionVO.startDate }" class="form-control showDay bir1" id="bir" placeholder="year" name="startDate" style="float:left; margin-right: 1%;">
				<input  type="date" value="${promotionVO.endDate }" class="form-control showDay bir2" id="bir" placeholder="year" name="endDate">
			</div>
			<div class="form-group">
				<label>티켓 구입처</label>
				<div style="margin-bottom: 10px;">
					<input name="consertTicket" value="${promotionVO.consertTicket }" type="text" class="form-control showInsertText" placeholder="ex)http://ticket.interpark.com/Ticket/Goods/GoodsInfo.asp?GoodsCode=17008482" style="float:left;width: 97%;margin-bottom: 15px;">
				</div>
			</div>
			<div class="form-group">
				<label>장소</label> 
				<div style="margin-bottom: 10px;">
					<input name="address" type="text" value="${detailAddress }" class="form-control showInsertText" id="address" placeholder="주소" style="float:left;width: 81%;">
					<input type="button" class="btn btn-default" onclick="execDaumPostcode()" value="주소 검색"><br>
				</div>
				<input name="detailAddress" type="text" value="${address }" class="form-control showInsertText" id="detailAddress" placeholder="상세주소">
		...
		<div style="float: right; margin-bottom: 50px;">
			<button onclick='saveContent()' type="button" class="btn btn-primary" style="width:90px;background-color:#26bbe2; margin-right: 10px;">수정</button>		
...
<script>
    ...
    var mapContainer = document.getElementById('map'), // 지도를 표시할 div
        mapOption = {
            center: new daum.maps.LatLng(geograp[0],geograp[1]), // 지도의 중심좌표
            level: 5 // 지도의 확대 레벨
        };
